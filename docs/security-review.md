...
|--------|------|---------|------|---------|
| Critical | 0件 | - | - | - |
| High | 2件 | 1件 | 1件 | ~~CSP無効化~~、未署名配布 |
| Medium | 3件 | 3件 | 0件 | ~~ReDoS~~、~~shell:allow-open~~、~~withGlobalTauri~~ |
| Low | 2件 | 1件 | 1件 | ~~エラー情報露出~~、XSS潜在リスク |
| **合計** | **7件** | **5件** | **2件** | |
...
|---|--------|--------|---------|-------|------|------|
| V-001 | CSP無効化 | High | セキュリティ設定不備 | A05 | D-001 | ✅ 対応済み |
| V-002 | ReDoS可能性 | Medium | 入力検証不備 | A03 | D-005 | ✅ 対応済み |
| V-003 | 未署名アプリケーション配布 | High | ソフトウェア完全性 | A08 | D-003 | Phase 2 |
| V-004 | エラーメッセージによる情報漏洩 | Low | 情報漏洩 | A01 | D-006 | ✅ 対応済み |
| V-005 | shell:allow-open権限によるURL開放 | Medium | 権限過剰 | A01 | D-004 | ✅ 対応済み |
| V-006 | withGlobalTauri有効化 | Medium | 攻撃面拡大 | A05 | D-002 | ✅ 対応済み |
| V-007 | ラベルデータのXSS潜在リスク | Low | XSS | A03 | D-007 | React保護 |
